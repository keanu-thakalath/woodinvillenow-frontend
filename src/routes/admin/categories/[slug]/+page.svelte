<script lang="ts">
    ...
    async function handleDeleteSubmit(e: SubmitEvent) {
        ...
        const res = await fetch(`http://${PUBLIC_BACKEND_DOMAIN}/api/categories/${data.category.url_slug}`, {
            method: 'DELETE',
            headers: {
        ...
        <form on:submit|preventDefault={handleSubmit} class="space-y-4">
            <div>
                <div class="relative mt-1">
                    <input bind:value={data.category.name} type="text" placeholder="Name">
            ...
            <div>
                <div class="relative mt-1">
                    <input bind:value={data.category.id} type="text" disabled placeholder="ID">
            ...
            <div>
                <div class="relative mt-1">
                    <input bind:value={data.category.url_slug} type="text" disabled placeholder="URL Slug">
            ...
            <Submit bind:this={submit} value="Update Category"/>
        </form>
        
        <form on:submit|preventDefault={handleDeleteSubmit} class="space-y-4">
            <Submit bind:this={deleteSubmit} value="Delete Category"/>
        </form>
    </div>
</section>